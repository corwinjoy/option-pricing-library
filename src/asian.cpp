...
static short arithmetic_asian_setup(
  ...
  if (stop <= t || T < INPUT_UNDERFLOW ) { /* Option has finished averaging */
  ...
  if (start < t) { /* Averaging has already started, make
	 ...
	 *X=((*X)-alpha*Past_Avg)/(*beta);
	 if ((*X)< 0.0) { /*This option is deeply in the money,
  ...
  if (vol <= 0.01) {/* Handle zero vol case */
   	if (call_put == OPT_CALL) {
			*value = exp(-r*(T-t))*MAX(alpha*Past_Avg+(*beta)*Fwd_Avg-oldX,0);
			return TRUE;
		} else
  ...
  if (start == 0.0) {  /* Compute Sa=E[M(t)] and D=E[M(t)^2] via <7b> and <8b> on
  ...
  else { /* Compute Sa=E[M(t)] and D=E[M(t)^2] via <7b> and <8b> on
				 p. 480 of Levy's "Pricing European currency options "
			 */
...
short arithmetic_asian(double Fwd_Avg, double Past_Avg, double X, double vol,
	...
	/* Check Inputs */
	assert (result != NULL);
	if ( Fwd_Avg <= 0 || X <= 0 || vol < 0 || r < 0  || q < 0
			|| (stop - start) < 0 || stop > T) {
		*result = 0.0;
		return FALSE;
	}

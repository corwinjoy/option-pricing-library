...
static short crit_bisect(double S, double X,
	...
	if ( f_min * f_max >= 0.0 ) { /* No root in this interval */
		*result = -1.0;
		return FALSE;
	}

	if ( f_min > 0.0){ /* Switch min & max s.t. min represents lower bound */
		temp = max;
		max = min;
		min = temp;
	}
	...
	for (i = 0; i < max_iter; i++) {
		...
		if (f_mid < 0.0)
			...
			max = mid;
		if ( fabs(max - min) < acc) { /* Check for convergence */
			*result =  (min + max) / 2.0;
			return TRUE;
		}
	}
...
short amer_call(double S, double X, double vol, double r, double q, double t,
   ...
   vol_rt_t =  vol * sqrt(t);
	...
	if (*crit_price == 0.0) {/* Critical Price unknown, must calc */
		...
		if (status == FALSE) { /* Failed to converge */
			*result = 0.0;
			return FALSE;
		}
	...
	if (S < S_star) {  /* Early exercise not optimal */
		d1 = fn_d1(S, X, drift, vol_rt_t);
		d2 = d1 - vol_rt_t;
		N_d1 = N(d1);
		c = S * disc_q * N_d1 - X * disc_r * N(d2);

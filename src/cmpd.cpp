...
static short euro_S(double price, double X, double vol, double r, double q, double t,
	...
	for (i=0; i<max_iter; i++) {
		...
		if (fabs(dx) < price_acc) {  /* Convergence */
			*result = guess;
			return TRUE;
		}
	}
...
static short euro_call_call(double S, double X1, double X2, double vol,
	...
	c = S * exp(-q*t2) * M(a1, b1, sqrt_t1_t2)
		- X2 * exp(-r*t2) * M(a2, b2, sqrt_t1_t2) - exp(-r*t1) * X1 * N(a2);
	*result = c;
...
static short euro_call_put(double S, double X1, double X2, double vol,
	...
	c = X2 * exp(-r*t2) * M(-a2, -b2, sqrt_t1_t2)
		 -S * exp(-q*t2) * M(-a1, -b1, sqrt_t1_t2) - exp(-r*t1) * X1 * N(-a2);
	*result = c;
...
static short euro_put_put(double S, double X1, double X2, double vol,
	...
	c = S * exp(-q*t2) * M(a1, -b1, -sqrt_t1_t2)
		 -X2 * exp(-r*t2) * M(a2, -b2, -sqrt_t1_t2)
		 + exp(-r*t1) * X1 * N(a2);
	*result = c;
